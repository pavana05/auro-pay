...
const AdminParentLinks = () => {
  ...
  const [stats, setStats] = useState({ active: 0, totalPocketMoney: 0, orphaned: 0 });

  const fetchData = async () => {
    setLoading(true);
    const { data } = await supabase.from("parent_teen_links").select("*, parent:profiles!parent_teen_links_parent_id_fkey(full_name, phone), teen:profiles!parent_teen_links_teen_id_fkey(full_name, phone)").order("created_at", { ascending: false });
    setLinks(data || []);
    const active = (data || []).filter((l: any) => l.is_active).length;
    const totalPM = (data || []).reduce((s: number, l: any) => s + (l.pocket_money_amount || 0), 0);
    // Count teens without parent links
    const { count: totalTeens } = await supabase.from("profiles").select("id", { count: "exact", head: true }).eq("role", "teen");
    const linkedTeens = new Set((data || []).map((l: any) => l.teen_id));
    setStats({ active, totalPocketMoney: totalPM, orphaned: (totalTeens || 0) - linkedTeens.size });
    setLoading(false);
  };
