...
const C = { cardBg: "#0f0720", border: "rgba(139,92,246,0.12)", primary: "#7c3aed", success: "#22c55e", warning: "#f59e0b", textPrimary: "#ffffff", textSecondary: "rgba(255,255,255,0.55)", textMuted: "rgba(255,255,255,0.3)" };

const AdminSavingsOversight = () => {
  const [goals, setGoals] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState({ active: 0, totalSaved: 0, completedMonth: 0, avgRate: 0 });

  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase.from("savings_goals").select("*, teen:profiles!savings_goals_teen_id_fkey(full_name)").order("created_at", { ascending: false });
      const g = data || [];
      setGoals(g);
      ...
      const now = new Date();
      const completedThisMonth = completed.filter((x: any) => {
      ...
      setStats({ active, totalSaved, completedMonth: completedThisMonth, avgRate });
      setLoading(false);
    };
    fetch();
  }, []);
